# agents-01 — 专家配置管理 (StaffDeck 融合)

> 对应文档: `docs/prd/agents-001-expert-management.md`
> 来源: StaffDeck `prd-001 数字员工管理` + `prd-014 开放广场`
> Status: 🟢 已完成 | 分支: `feature/expert-management`
...
Plan 06 覆盖插件/Skill/MCP/Expert 的**使用侧**（浏览、安装、召唤）。Plan agents-01 覆盖**管理侧**（创建、编辑、绑定、发布）。两者共享数据，在 ExpertCenter 同一个面板内通过 Tab 切换。

| 维度 | Plan 06 (不改) | Plan agents-01 (新增) |
|------|---------------|----------------|
...
新增:
  docs/prd/agents-001-expert-management.md     ← PRD 文档
  docs/README.md                            ← 文档索引
...
### 1. 数据模型扩展 (`src/lib/expert-types.ts`)
...
### 2. ExpertCenter 重构 (`src/components/ExpertCenter.tsx`)
...
### 3. 专家编辑弹窗 (`src/components/ExpertEditorModal.tsx`)
...
### 4. 广场 (`src/components/ExpertSquare.tsx`)
...
| 批次 | 内容 | 提交信息 |
|:---:|------|------|
| 1 | `expert-types.ts` 数据模型扩展 | `docs: add agents-001 & extend expert data model` |
...
- [x] 管理 Tab 可查看/筛选/新建/编辑/删除专家
- [x] 专家编辑弹窗可绑定 SOP/Skill/MCP/知识库/连接器/模型
- [x] Test Run 可测试当前配置的对话效果
