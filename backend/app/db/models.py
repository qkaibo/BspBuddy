...
class ModelConfig(SQLModel, table=True):
    ...
    enabled: bool = True
    # ---- New: BYOK / tenant-shared fields ----
    storage_mode: str = Field(default="cloud", index=True)  # "cloud" | "local"
    scope: str = Field(default="personal", index=True)  # "personal" | "tenant"
    created_at: datetime = Field(default_factory=utc_now)
    updated_at: datetime = Field(default_factory=utc_now)


class ExpertModelCatalog(SQLModel, table=True):
    """Admin-managed model catalog for expert agents (separate from user ModelConfig)."""
