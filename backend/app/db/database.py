...
def _migrate_expert_model_catalog_schema(conn, inspector, tables: set[str]) -> None:
    """Add expert_model_catalog_id column to agent_model_bindings and ensure model_config_id is nullable."""
    if "agent_model_bindings" not in tables:
        return
    columns = {col["name"] for col in inspector.get_columns("agent_model_bindings")}
    if "expert_model_catalog_id" not in columns:
        conn.execute(
            text("ALTER TABLE agent_model_bindings ADD COLUMN expert_model_catalog_id VARCHAR")
        )
    # Add is_default column to expert_model_catalogs
    if "expert_model_catalogs" in tables:
        cat_columns = {col["name"] for col in inspector.get_columns("expert_model_catalogs")}
        if "is_default" not in cat_columns:
            conn.execute(
                text("ALTER TABLE expert_model_catalogs ADD COLUMN is_default BOOLEAN NOT NULL DEFAULT 0")
            )
    # Ensure model_config_id is nullable (SQLite requires table recreation)
    col_info = {col["name"]: col for col in inspector.get_columns("agent_model_bindings")}
...
def _recreate_agent_model_bindings_nullable(conn, inspector) -> None:
    """Recreate agent_model_bindings with model_config_id as nullable."""
    migration_key = "expert_model_catalog_nullable_mc_20260809"
    conn.execute(
        text("CREATE TABLE IF NOT EXISTS _model_migrations (key VARCHAR PRIMARY KEY, value VARCHAR)")
    )
    pending = conn.execute(
        text("SELECT value FROM _model_migrations WHERE key = :key"),
        {"key": migration_key},
    ).first()
    if pending:
        return
    # Get current columns
    cols = inspector.get_columns("agent_model_bindings")
    col_names = [c["name"] for c in cols]
    # Recreate table with all columns nullable
    conn.execute(
        text("CREATE TABLE agent_model_bindings_new ("
             "id VARCHAR PRIMARY KEY,"
             "tenant_id VARCHAR,"
             "agent_id VARCHAR,"
             "role VARCHAR DEFAULT 'default',"
             "model_config_id VARCHAR,"
             "expert_model_catalog_id VARCHAR,"
             "created_at VARCHAR,"
             "updated_at VARCHAR"
             ")")
    )
    rows = conn.execute(
        text("SELECT id, tenant_id, agent_id, role, model_config_id, "
             "expert_model_catalog_id, created_at, updated_at "
             "FROM agent_model_bindings")
    ).fetchall()
    for row in rows:
        conn.execute(
            text("INSERT INTO agent_model_bindings_new VALUES (?,?,?,?,?,?,?,?)"),
            row,
        )
    conn.execute(text("DROP TABLE agent_model_bindings"))
    conn.execute(text("ALTER TABLE agent_model_bindings_new RENAME TO agent_model_bindings"))
    conn.execute(
        text("CREATE UNIQUE INDEX IF NOT EXISTS uq_agent_model_binding "
             "ON agent_model_bindings(tenant_id, agent_id, role)")
    )
    conn.execute(
        text("INSERT INTO _model_migrations (key, value) VALUES (:key, '1')"),
        {"key": migration_key},
    )
