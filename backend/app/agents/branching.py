...
def model_for_agent(
    ...
    if agent:
        for candidate_role in roles:
            ...
            if binding:
                # Prefer ExpertModelCatalog (new system)
    ...
    # Last resort: user-level default model config
    model = db.exec(
...
def _runtime_model_from_catalog(
    tenant_id: str, catalog: ExpertModelCatalog
) -> ResolvedModelConfig:
    """Build a ResolvedModelConfig from an ExpertModelCatalog entry."""
    from types import MappingProxyType

    api_key = decrypt_secret(catalog.api_key_encrypted)
    try:
        protocol = ModelApiProtocol(catalog.api_protocol)
    except ValueError:
        protocol = ModelApiProtocol.OPENAI_CHAT_COMPLETIONS
    return ResolvedModelConfig(
        id=catalog.id,
        tenant_id=tenant_id,
        api_protocol=protocol,
        base_url=catalog.base_url,
        api_key_encrypted=catalog.api_key_encrypted,
        model=catalog.model,
        temperature=catalog.temperature,
        max_output_tokens=catalog.max_output_tokens,
        protocol_options=MappingProxyType({}),
        legacy_extra_body=MappingProxyType({}),
        config_revision=1,
        security_revision=1,
        purpose="runtime",
