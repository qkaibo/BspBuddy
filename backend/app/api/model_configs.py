...
def list_model_configs(
    ...
    # Return personal models + tenant-shared models visible to all members
    rows = db.exec(
        select(ModelConfig).where(
            ModelConfig.tenant_id == tenant_id,
            (ModelConfig.scope == "personal") | (ModelConfig.scope == "tenant"),
        )
    ).all()
...
def update_model_config(
    ...
    for field in ("name", "base_url", "model", "temperature", "max_output_tokens", "storage_mode", "scope"):
        value = getattr(request, field)
