...
def create_agent(
    ...
    row = AgentProfile(
        ...
        status="active",
        metadata_json=_metadata_with_creator(
            _enrich_metadata_with_bindings(request.metadata or {}, request.bindings, request.bspbuddy_status),
            user,
        ),
    )
    db.add(row)
...
def update_agent(
    ...
    if request.metadata is not None or request.bindings is not None or request.bspbuddy_status is not None:
        next_meta = _enrich_metadata_with_bindings(
            request.metadata or {}, request.bindings, request.bspbuddy_status
        )
        row.metadata_json = _metadata_preserving_creator(
            row.metadata_json or {}, next_meta, user
        )
...
def update_agent_models(
    ...
    agent = _get_agent(db, request.tenant_id, agent_id)
    # Must validate the ExpertModelCatalog entry is from the same tenant
    # before allowing the binding, even for non-admin users.
    for item in request.bindings:
        if item.expert_model_catalog_id:
            catalog_entry = db.get(ExpertModelCatalog, item.expert_model_catalog_id)
            if catalog_entry is None or catalog_entry.tenant_id != request.tenant_id:
                raise HTTPException(status_code=400, detail="Invalid expert model catalog entry")
    ...
    for item in request.bindings:
        ...
        if existing:
            ...
            if item.expert_model_catalog_id is not None:
                existing.expert_model_catalog_id = item.expert_model_catalog_id
        ...
        db.add(
            AgentModelBinding(
                ...
                expert_model_catalog_id=item.expert_model_catalog_id,
            )
        )
    db.commit()
...
def _enrich_metadata_with_bindings(
    metadata: dict[str, object],
    bindings: dict[str, object] | None,
    bspbuddy_status: str | None,
) -> dict[str, object]:
    """Merge BspBuddy bindings and status into the metadata dict for storage."""
    enriched = dict(metadata or {})
    if bindings is not None:
        enriched["bindings"] = bindings
    if bspbuddy_status is not None:
        enriched["bspbuddy_status"] = bspbuddy_status
    return enriched
