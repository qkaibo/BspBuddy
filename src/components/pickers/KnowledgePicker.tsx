...
export function KnowledgePicker({ boundIds, onChange, expertId: _expertId }: Props) {
  ...
  useEffect(() => {
    let cancelled = false
    setLoading(true)
    ipc.invoke(IPC_CHANNELS.KNOWLEDGE_LIST)
      .then((data) => {
        if (cancelled) return
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
        <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-primary)' }}>绑定知识库</div>
        <button
          onClick={() => console.log('Navigate to Knowledge panel')}
          style={{
      ...
      {boundIds.length > 0 && (
        <div style={{ marginBottom: 16 }}>
          <div style={{ fontSize: 11, color: 'var(--text-tertiary)', marginBottom: 8 }}>已绑定知识库:</div>
      ...
      <div>
        <div style={{ fontSize: 11, color: 'var(--text-tertiary)', marginBottom: 8 }}>可选知识库:</div>
