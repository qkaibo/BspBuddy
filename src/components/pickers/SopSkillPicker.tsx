...
export function SopSkillPicker({ boundIds, onChange, expertId: _expertId }: Props) {
  ...
  useEffect(() => {
    let cancelled = false
    setLoading(true)
    ipc.invoke(IPC_CHANNELS.SOP_LIST)
      .then((data) => {
        if (cancelled) return
        const raw = Array.isArray(data) ? data : (data as { items?: ResourceSummary[] })?.items
        const list = (Array.isArray(raw) ? raw : []).map((r) => ({
          ...r,
          // normalize aliases for ResourceCard
          skill_id: (r as { skillId?: string; skill_id?: string }).skillId
            || (r as { skill_id?: string }).skill_id,
          business_domain: (r as { businessDomain?: string; business_domain?: string }).businessDomain
            || (r as { business_domain?: string }).business_domain,
        })) as ResourceSummary[]
        setAvailable(list)
      })
      .catch(() => {
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
        <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-primary)' }}>绑定 SOP 技能</div>
        <button
          onClick={() => console.log('Navigate to SOP panel')}
          style={{
      ...
      {boundIds.length > 0 && (
        <div style={{ marginBottom: 16 }}>
          <div style={{ fontSize: 11, color: 'var(--text-tertiary)', marginBottom: 8 }}>已绑定 SOP:</div>
      ...
      <div>
        <div style={{ fontSize: 11, color: 'var(--text-tertiary)', marginBottom: 8 }}>可选 SOP:</div>
