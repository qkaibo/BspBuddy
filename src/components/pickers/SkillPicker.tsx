...
export function SkillPicker({ boundIds, onChange, expertId: _expertId }: Props) {
  ...
  useEffect(() => {
    let cancelled = false
    setLoading(true)
    ipc.invoke(IPC_CHANNELS.SKILL_LIST)
      .then((data) => {
        if (cancelled) return
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
        <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-primary)' }}>绑定 Skill</div>
        <button
          onClick={() => console.log('Navigate to Skill panel')}
          style={{
      ...
      {boundIds.length > 0 && (
        <div style={{ marginBottom: 16 }}>
          <div style={{ fontSize: 11, color: 'var(--text-tertiary)', marginBottom: 8 }}>已绑定 Skill:</div>
      ...
      <div>
        <div style={{ fontSize: 11, color: 'var(--text-tertiary)', marginBottom: 8 }}>可选 Skill:</div>
